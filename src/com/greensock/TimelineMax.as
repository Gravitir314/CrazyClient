...
public class TimelineMax extends TimelineLite implements IEventDispatcher
{
	...
	public function removeCallback(callback:Function, position:* = null):TimelineMax
	{
		if (callback != null)
		{
			...
			{
				var a:Array = getTweensOf(callback, false), i:int = a.length, time:Number = _parseTimeOrLabel(position);
				while (--i > -1)
				{
	...
	override public function render(time:Number, suppressEvents:Boolean = false, force:Boolean = false):void
	{
		...
		var totalDur:Number = (!_dirty) ? _totalDuration : totalDuration(), prevTime:Number = _time,
				prevTotalTime:Number = _totalTime, prevStart:Number = _startTime, prevTimeScale:Number = _timeScale,
				prevRawPrevTime:Number = _rawPrevTime, prevPaused:Boolean = _paused, prevCycle:int = _cycle,
		...
		if (_cycle != prevCycle) if (!_locked)
		{
			...
			var backwards:Boolean = (_yoyo && (prevCycle & 1) !== 0),
					wrap:Boolean = (backwards == (_yoyo && (_cycle & 1) !== 0)), recTotalTime:Number = _totalTime,
					recCycle:int = _cycle, recRawPrevTime:Number = _rawPrevTime, recTime:Number = _time;
		...
		{
			if (prevTotalTime !== _totalTime) if (_onUpdate != null) if (!suppressEvents)
		...
		if (callback) if (!_locked) if (!_gc) if (prevStart === _startTime || prevTimeScale !== _timeScale) if (_time === 0 || totalDur >= totalDuration())
	...
	{
		var a:Array = [], all:Array = getChildren(nested, tweens, timelines), cnt:int = 0, l:int = all.length, i:int,
				tween:Animation;
		for (i = 0; i < l; i++)
		{
			tween = all[i];
	...
	public function getLabelAfter(time:Number = NaN):String
	{
		...
		var labels:Array = getLabelsArray(), l:int = labels.length, i:int;
		for (i = 0; i < l; i++)
		{
	...
	public function getLabelBefore(time:Number = NaN):String
	{
		...
		var labels:Array = getLabelsArray(), i:int = labels.length;
		while (--i > -1)
		{
			if (labels[i].time < time)
			{
	...
	{
		var a:Array = [], cnt:int = 0, p:String;
		for (p in _labels)
		{
