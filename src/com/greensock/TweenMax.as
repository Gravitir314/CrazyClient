...
public class TweenMax extends TweenLite implements IEventDispatcher
{
		...
		if (_initted)
		{
			...
			else
			{
				...
				else if (_time > 0)
				{
					_initted = false;
					_init();
					var inv:Number = 1 / (1 - curRatio), pt:PropTween = _firstPT, endValue:Number;
					while (pt)
					{
						endValue = pt.s + pt.c;
						pt.c *= inv;
	...
	{
		if (!_initted) if (_duration === 0 && vars.repeat)
		...
		var totalDur:Number = (!_dirty) ? _totalDuration : totalDuration(), prevTime:Number = _time,
				prevTotalTime:Number = _totalTime, prevCycle:Number = _cycle, isComplete:Boolean, callback:String,
				pt:PropTween, rawPrevTime:Number;
		if (time >= totalDur)
		{
			_totalTime = totalDur;
			_cycle = _repeat;
...
//---- STATIC FUNCTIONS -----------------------------------------------------------------------------------------------------------
	...
	public static function staggerTo(targets:Array, duration:Number, vars:Object, stagger:Number = 0, onCompleteAll:Function = null, onCompleteAllParams:Array = null):Array
	{
		...
		var a:Array = [], l:int = targets.length, delay:Number = vars.delay || 0, copy:Object, i:int, p:String;
		for (i = 0; i < l; i++)
		{
			copy = {};
			for (p in vars)
	...
	protected static function _getChildrenOf(timeline:SimpleTimeline, includeTimelines:Boolean):Array
	{
		...
		var a:Array = [], cnt:int = 0, tween:Animation = timeline._first;
		while (tween)
		{
	...
	{
		var a:Array = getAllTweens(timelines), l:int = a.length, isDC:Boolean,
				allTrue:Boolean = (tweens && delayedCalls && timelines), tween:Animation, i:int;
		for (i = 0; i < l; i++)
		{
			tween = a[i];
	...
	{
		var a:Array = getAllTweens(false), l:int = a.length, i:int;
		for (i = 0; i < l; i++)
		{
	...
	{
		var a:Array = getAllTweens(timelines), isDC:Boolean, tween:Animation,
				allTrue:Boolean = (tweens && delayedCalls && timelines), i:int = a.length;
		while (--i > -1)
		{
			tween = a[i];
...
//---- GETTERS / SETTERS ----------------------------------------------------------------------------------------------------------
	...
	public static function globalTimeScale(value:Number = NaN):Number
	{
		...
		var tl:SimpleTimeline = _rootTimeline, t:Number = (getTimer() / 1000);
