...
//---- EVENT HANDLERS ------------------------------------------------------------------------------------
	...
	protected function _metaDataHandler(info:Object):void
	{
		...
		{
			var a:Array = this.metaData.cuePoints, i:int = a.length;
			while (--i > -1)
			{
	...
	protected function _playProgressHandler(event:Event):void
	{
		...
		{ //remember, bufferLength could be less than bufferTime if videoTime is towards the end of the video and there's less time remaining to play than there is bufferTime. 
			_onBufferFull();
		}
		if (_bufferFull && (_firstCuePoint || _dispatchPlayProgress))
		{
			var prevTime:Number = _prevTime, prevCueTime:Number = _prevCueTime;
			...
			var next:CuePoint, cp:CuePoint = _firstCuePoint;
			while (cp)
			{
				next = cp.next;
	...
	{
		_renderedOnce = true;
		if (!_videoPaused || _initted) if (!_finalFrame)
