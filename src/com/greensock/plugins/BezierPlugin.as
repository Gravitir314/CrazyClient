...
public class BezierPlugin extends TweenPlugin
{
	...
	override public function _onInitTween(target:Object, value:*, tween:TweenLite):Boolean
	{
		...
		var values:Array = vars.values || [], first:Object = {}, second:Object = values[0],
				autoRotate:Object = vars.autoRotate || tween.vars.orientToBezier, p:String, isFunc:Boolean, i:int,
				j:int, ar:Array, prepend:Object;
	...
	{
		var obj:Object = {}, first:Object = prepend || values[0], props:Array, i:int, p:String, j:int, a:Array, l:int,
				r:Number, seamless:Boolean, last:Object;
		correlate = "," + correlate + ",";
		if (first is Point)
		{
			props = ["x", "y"];
	...
	{
		type = type || "soft";
		var obj:Object = {}, inc:int = (type === "cubic") ? 3 : 2, soft:Boolean = (type === "soft"), a:Number, b:Number,
				c:Number, d:Number, cur:Array, props:Array, i:int, j:int, l:int, p:String, cnt:int, tmp:Object;
		if (soft && prepend)
		{
	...
	{
		var a:Array = [], l:int, i:int, p1:Number, p2:Number, p3:Number, tmp:Object;
		if (prepend)
		{
			values = [prepend].concat(values);
			i = values.length;
	...
	{
		var l:int = a.length - 1, ii:int = 0, cp1:Number = a[0].a, i:int, p1:Number, p2:Number, p3:Number, seg:Segment,
				m1:Number, m2:Number, mm:Number, cp2:Number, qb:Array, r1:Number, r2:Number, tl:Number;
		for (i = 0; i < l; i++)
		{
			seg = a[ii];
			p1 = seg.a;
	...
	{
		var q1:Object = {a: a}, q2:Object = {}, q3:Object = {}, q4:Object = {c: d}, mab:Number = (a + b) / 2,
				mbc:Number = (b + c) / 2, mcd:Number = (c + d) / 2, mabc:Number = (mab + mbc) / 2,
				mbcd:Number = (mbc + mcd) / 2, m8:Number = (mbcd - mabc) / 8;
		q1.b = mab + (a - mab) / 4;
		q2.b = mabc + m8;
	...
	{
		var a:Array = [], lengths:Array = [], d:Number = 0, total:Number = 0, threshold:int = precision - 1,
				segments:Array = [], curLS:Array = [], //current length segments array
	...
	{
		var inc:Number = 1 / precision, j:int = a.length, d:Number, d1:Number, s:Number, da:Number, ca:Number,
				ba:Number, p:Number, i:int, inv:Number, bez:Segment, index:int;
		while (--j > -1)
		{
			bez = a[j];
			s = bez.a;
	...
	{
		var a:Array = this._props, p:String, i:int;
		for (p in _beziers)
		{
			if (p in lookup)
			{
	...
	{
		var op:Array = this._overwriteProps, i:int = op.length;
		while (--i > -1)
		{
	...
	{
		var segments:int = this._segCount, func:Object = this._func, target:Object = this._target,
				notStart:Boolean = (v !== this._startRatio), curIndex:int, inv:Number, i:int, p:String, b:Segment,
				t:Number, val:Number, l:int, lengths:Array, curSeg:Array;
		if (this._timeRes == 0)
		{
		...
		{
			var ar:Array = this._autoRotate, b2:Segment, x1:Number, y1:Number, x2:Number, y2:Number, add:Number,
					conv:Number;
			i = ar.length;
			while (--i > -1)
			{
