...
public class ScoringBox extends Sprite 
    {
        ...
        public function ScoringBox(_arg_1:Rectangle, _arg_2:XML)
        {
            ...
            var _local_4:BitmapData = FameUtil.getFameIcon();
            _local_4 = BitmapUtil.cropToBitmapData(_local_4, 6, 6, (_local_4.width - 12), (_local_4.height - 12));
            this.addLine(TextKey.FAMEVIEW_BASEFAMEEARNED, null, 0, _arg_2.BaseFame, true, 0xFFC800, "", "", new Bitmap(_local_4));
        ...
        {
            var _local_2:Number;
            var _local_3:ScoreTextLine;
            var _local_4:int;
            var _local_5:Number = (this.startTime_ + ((2000 * (this.scoreTextLines_.length - 1)) / 2));
            _local_2 = getTimer();
            var _local_6:int = int(Math.min(this.scoreTextLines_.length, (((2 * (getTimer() - this.startTime_)) / 2000) + 1)));
            while (_local_4 < _local_6)
            {
                _local_3 = this.scoreTextLines_[_local_4];
