...
public class FameUtil
    {
        ...
        {
            var _local_3:int;
            var _local_4:int = Math.max(_arg_1, _arg_2);
            while (_local_3 < STARS.length)
            {
        ...
        public static function numAllTimeStars(_arg_1:int, _arg_2:int, _arg_3:XML):int
        {
            ...
            var _local_5:int;
            var _local_6:int;
            for each (_local_4 in _arg_3.ClassStats)
            {
                if (_arg_1 == int(_local_4.@objectType))
                {
                    _local_6 = int(_local_4.BestFame);
                }
                else
                {
