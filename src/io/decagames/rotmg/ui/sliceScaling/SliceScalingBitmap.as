...
public class SliceScalingBitmap extends Bitmap
    {
        ...
        private function prepare9grid():void
        {
            ...
            var _local_2:int = (this.currentWidth - (this.scaleGrid.x * 2));
            while (_local_3 < _local_2)
            {
                this.bitmapData.copyPixels(this.bitmapDataToSlice, new Rectangle(this.scaleGrid.x, 0, this.scaleGrid.width, this.scaleGrid.y), new Point(((this.scaleGrid.x + _local_3) + this.margin.x), this.margin.y));
                _local_3++;
            };
            _local_3 = 0;
            while (_local_3 < _local_2)
            {
                this.bitmapData.copyPixels(this.bitmapDataToSlice, new Rectangle(this.scaleGrid.x, (this.scaleGrid.y + this.scaleGrid.height), this.scaleGrid.width, this.scaleGrid.y), new Point(((this.scaleGrid.x + _local_3) + this.margin.x), ((this.currentHeight - this.scaleGrid.y) + this.margin.y)));
                _local_3++;
            };
            var _local_4:int = (this.currentHeight - (this.scaleGrid.y * 2));
            _local_3 = 0;
            while (_local_3 < _local_4)
            {
                this.bitmapData.copyPixels(this.bitmapDataToSlice, new Rectangle(0, this.scaleGrid.y, this.scaleGrid.x, this.scaleGrid.height), new Point(this.margin.x, ((this.scaleGrid.y + _local_3) + this.margin.y)));
                _local_3++;
            };
            _local_3 = 0;
            while (_local_3 < _local_4)
            {
                this.bitmapData.copyPixels(this.bitmapDataToSlice, new Rectangle((this.scaleGrid.x + this.scaleGrid.width), this.scaleGrid.y, this.scaleGrid.x, this.scaleGrid.height), new Point(((this.currentWidth - this.scaleGrid.x) + this.margin.x), ((this.scaleGrid.y + _local_3) + this.margin.y)));
                _local_3++;
            };
            var _local_5:uint = this.bitmapDataToSlice.getPixel32((this.scaleGrid.x + this.scaleGrid.width), (this.scaleGrid.y + this.scaleGrid.height));
            this.bitmapData.lock();
            _local_3 = this.scaleGrid.x;
            while (_local_3 < (this.currentWidth - this.scaleGrid.x))
            {
                _local_1 = this.scaleGrid.y;
                while (_local_1 < (this.currentHeight - this.scaleGrid.y))
                {
                    this.bitmapData.setPixel32((_local_3 + this.margin.x), (_local_1 + this.margin.y), _local_5);
                    _local_1++;
                };
                _local_3++;
            };
