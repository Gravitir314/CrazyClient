...
public class UIGrid extends Sprite
    {
        ...
        public function addDecoration(_arg_1:String):void
        {
            ...
            var _local_7:int;
            _local_2 = null;
            var _local_4:int = int(((this.gridWidth - ((this.numberOfColumns - 1) * this.gridMargin)) / this.numberOfColumns));
            var _local_5:int = (this.numberOfColumns - 1);
            ...
            while (_local_7 < _local_6)
            {
                ...
                {
                    _local_2 = TextureParser.instance.getSliceScalingBitmap("UI", _arg_1);
                    _local_2.x = (((_local_3 * (this.gridMargin / 2)) + ((_local_3 + 1) * (_local_4 + (this.gridMargin / 2)))) - (_local_2.width / 2));
                    _local_2.y = (184 - (_local_2.height / 2));
                    this.gridContent.addChild(_local_2);
                    this.decors.push(_local_2);
                    _local_3++;
                };
                _local_7++;
            };
        ...
        public function render():void
        {
            ...
            var _local_3:int = int(((this.gridWidth - ((this.numberOfColumns - 1) * this.gridMargin)) / this.numberOfColumns));
            var _local_4:int = 1;
            var _local_8:int = int(Math.ceil((this.elements.length / this.numberOfColumns)));
            var _local_9:int = 1;
            for each (_local_1 in this.elements)
            {
                _local_1.resize(_local_3);
                if (_local_1.height > _local_7)
                {
                    _local_7 = _local_1.height;
                };
                _local_1.x = _local_5;
                _local_1.y = _local_6;
                if (++_local_4 > this.numberOfColumns)
                {
                    _local_9++;
                    _local_5 = 0;
                    if (((_local_9 == _local_8) && (this._centerLastRow)))
                    {
                        _local_2 = ((_local_9 * this.numberOfColumns) - this.elements.length);
                        _local_5 = int(Math.round((((_local_2 * _local_3) + ((_local_2 - 1) * this.gridMargin)) / 2)));
                    };
                    _local_6 = (_local_6 + (_local_7 + this.gridMargin));
                    _local_7 = 0;
                    _local_4 = 1;
                }
                else
                {
                    _local_5 = (_local_5 + (_local_3 + this.gridMargin));
                };
            };
            if (this.scrollHeight != -1)
            {
                ...
                this.gridMask.graphics.drawRect(0, 0, _local_3, this.scrollHeight);
