...
public class Model3D_stage3d
    {
        ...
        public function readBytes(_arg_1:ByteArray):void
        {
            ...
            this.dispose();
            var _local_9:* = "";
            var _local_10:Vector.<Number> = new Vector.<Number>();
            var _local_11:Vector.<Number> = new Vector.<Number>();
            var _local_12:Vector.<Number> = new Vector.<Number>();
            _arg_1.position = 0;
            var _local_13:String = _arg_1.readUTFBytes(_arg_1.bytesAvailable);
            var _local_14:Array = _local_13.split(/[\r\n]+/);
            for each (_local_4 in _local_14)
            {
                ...
                if (!((_local_4 == "") || (_local_4.charAt(0) === "#")))
                {
                    ...
                    {
                        case "v":
                            _local_10.push(parseFloat(_local_5[1]), parseFloat(_local_5[2]), parseFloat(_local_5[3]));
                            break;
                        case "vn":
                            _local_11.push(parseFloat(_local_5[1]), parseFloat(_local_5[2]), parseFloat(_local_5[3]));
                            break;
                        case "vt":
                            _local_12.push(parseFloat(_local_5[1]), (1 - parseFloat(_local_5[2])));
                            break;
                        case "f":
                            ...
                            break;
                        case "g":
                            _local_3 = new OBJGroup(_local_5[1], _local_9);
                            this.groups.push(_local_3);
                            break;
                        case "o":
                            this.name = _local_5[1];
                            break;
                        case "mtllib":
                            break;
                        case "usemtl":
            ...
            for each (_local_3 in this.groups)
            {
                ...
                {
                    _local_7 = (_local_2.length - 1);
                    _local_8 = 1;
                    while (_local_8 < _local_7)
                    {
                        _local_3._indices.push(this.mergeTuple(_local_2[_local_8], _local_10, _local_11, _local_12));
