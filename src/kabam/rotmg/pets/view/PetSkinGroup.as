...
public class PetSkinGroup extends Sprite 
    {
        ...
        {
            var _local_1:uint;
            var _local_2:PetIcon;
            var _local_3:PetItem;
            var _local_4:FeedFuseSlot;
            var _local_5:NativeSignal;
            var _local_6:uint = this.petSkinGroupVO.icons.length;
            _local_1 = 0;
            while (_local_1 < _local_6)
            {
                _local_2 = this.createPetIcon(this.petSkinGroupVO.icons[_local_1], 48);
                _local_3 = new PetItem();
                _local_3.setPetIcon(_local_2);
                _local_4 = new FeedFuseSlot();
                _local_4.mouseChildren = false;
                _local_4.setIcon(_local_3);
                _local_5 = new NativeSignal(_local_4, MouseEvent.CLICK, MouseEvent);
                _local_5.add(this.onSkinClicked);
                if ((_local_1 < 4))
                {
                    this.addToUpper(_local_4);
                }
                else
                {
