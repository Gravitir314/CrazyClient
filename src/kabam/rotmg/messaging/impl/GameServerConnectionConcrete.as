...
public class GameServerConnectionConcrete extends GameServerConnection
{
	...
	override public function playerText(_arg_1:String):void
	{
		...
		var _local_16:String = Parameters.data_.conCom;
		var _local_17:PlayerText = (this.messages.require(PLAYERTEXT) as PlayerText);
		_local_17.text_ = _arg_1;
		var _local_18:Array = _local_17.text_.toLowerCase().match((_local_16 + " ?(\\w*) ?(\\w*) ?(\\w*)"));
		var _local_19:int = -2;
		var _local_20:int = charId_;
		if (_local_18 != null)
		{
			_local_5 = 1;
			while (_local_5 < 4)
			{
				if (_local_18[_local_5] != "")
				{
					if (_local_5 > 1)
					{
						_local_15 = false;
					}
					if (_local_18[_local_5].substr(0, 1) == "v")
					{
						_local_19 = -5;
					}
					else
					{
						if (_local_18[_local_5] == "p")
						{
						...
						{
							if ((((_local_18[_local_5].length > 2) && ((((_local_18[_local_5].substr(0, 2) == "eu") || (_local_18[_local_5].substr(0, 2) == "us")) || (_local_18[_local_5].substr(0, 3) == "ase")) || (_local_18[_local_5].substr(0, 3) == "aus"))) || ((_local_18[_local_5].length == 2) && (_local_18[_local_5].substr(0, 2) == "ae"))))
							{
								_local_4 = 0;
								while (_local_4 < this.abbrs.length)
								{
									if (_local_18[_local_5] == this.abbrs[_local_4])
									{
							...
							else
							{
								...
								while (_local_10 < _local_6.length)
								{
									...
									if (((_local_8.substr((_local_8.length - 1), 1) == "2") && (_local_9.substr((_local_9.length - 1), 1) == "2")))
									{
										...
										{
											_local_18[_local_5] = _local_7[_local_10];
											break;
										}
									}
									else
								...
								else
								{
									...
									{
										if (_local_18[_local_5] == this.realms[_local_4].substring(0, _local_18[_local_5].length).toLowerCase())
										{
			...
			if (sRec)
			{
				...
				Parameters.data_.reconGID = _local_19;
			...
			gs_.dispatchEvent(new ReconnectEvent(_local_2, _local_19, false, _local_20, -1, new ByteArray(), false));
			return;
		}
		serverConnection.sendMessage(_local_17);
	}
	...
	override public function useItem(_arg_1:int, _arg_2:int, _arg_3:int, _arg_4:int, _arg_5:Number, _arg_6:Number, _arg_7:int):void
	{
		...
		{
			_local_8 = ObjectLibrary.propsLibrary_[this.player.equipment_[_arg_3]];
			if (((_local_8) && (_local_8.isPotion_)))
			{
				return;
			}
		}
		var _local_9:UseItem = (this.messages.require(USEITEM) as UseItem);
		_local_9.time_ = _arg_1;
		_local_9.slotObject_.objectId_ = _arg_2;
		_local_9.slotObject_.slotId_ = _arg_3;
		_local_9.slotObject_.objectType_ = _arg_4;
		_local_9.itemUsePos_.x_ = _arg_5;
		_local_9.itemUsePos_.y_ = _arg_6;
		_local_9.useType_ = _arg_7;
	...
	override public function useItem_new(_arg_1:GameObject, _arg_2:int):Boolean
	{
		...
		{
			_local_3 = ObjectLibrary.propsLibrary_[_arg_1.equipment_[_arg_2]];
			if (((_local_3) && (_local_3.isPotion_)))
			{
				return (false);
			}
		}
		var _local_4:int = _arg_1.equipment_[_arg_2];
		var _local_5:XML = ObjectLibrary.xmlLibrary_[_local_4];
		if ((((_local_5) && (!(_arg_1.isPaused()))) && ((_local_5.hasOwnProperty("Consumable")) || (_local_5.hasOwnProperty("InvUse")))))
		{
			if (!this.validStatInc(_local_4, _arg_1))
			{
				this.addTextLine.dispatch(ChatMessage.make("", (_local_5.attribute("id") + " not consumed. Already at max.")));
				return (false);
			}
			this.applyUseItem(_arg_1, _arg_2, _local_4, _local_5);
			SoundEffectLibrary.play("use_potion");
			return (true);
		}
		if (this.swapEquip(_arg_1, _arg_2, _local_5))
		{
			return (true);
		}
	...
	private function onConnected():void
	{
		...
		var _local_3:ReconnectEvent = new ReconnectEvent(new Server().setName("Daily Quest Room").setAddress(this.server_.address).setPort(this.server_.port), -11, false, charId_, _local_2.keyTime_, _local_2.key_, isFromArena_);
		...
		for each (_local_4 in _arg_2)
		{
			...
			switch (_local_4.statType_)
			{
				...
				case StatData.TEXTURE_STAT:
					if (!Parameters.data_.showSkins) break;
					if (_local_9 != null)
					{
