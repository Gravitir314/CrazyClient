...
public class MysteryBoxPanel extends Panel
    {
        ...
        {
            var _local_1:ShowPopupSignal;
            var _local_2:Injector = StaticInjectorContext.getInjector();
            var _local_3:MysteryBoxModel = _local_2.getInstance(MysteryBoxModel);
            var _local_4:Account = _local_2.getInstance(Account);
            var _local_5:OpenDialogSignal = _local_2.getInstance(OpenDialogSignal);
            if (((_local_3.isInitialized()) && (_local_4.isRegistered())))
            {
                if (ShopConfiguration.USE_NEW_SHOP)
                {
                    _local_1 = _local_2.getInstance(ShowPopupSignal);
