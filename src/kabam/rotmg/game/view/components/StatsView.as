...
public class StatsView extends Sprite
{
	...
	{
		var _local_1:StatView;
		var _local_2:int;
		var _local_3:int;
		while (_local_3 < statsModel.length)
		{
			_local_1 = this.createStat(_local_3, _local_2);
			this.stats_.push(_local_1);
	...
	private function createStat(_arg_1:int, _arg_2:int):StatView
	{
		...
		var _local_4:StatModel = statsModel[_arg_1];
		_local_3 = new StatView(_local_4.name, _local_4.abbreviation, _local_4.description, _local_4.redOnZero);
		_local_3.x = (((_arg_1 % 2) * this.WIDTH) / 2);
		_local_3.y = (_arg_2 * (this.HEIGHT / 3));
		return (_local_3);
	}
