...
public class WebChangePasswordDialogForced extends Frame
    {
        ...
        private function isNewPasswordValid():Boolean
        {
            var _local_1:String;
            var _local_2:int;
            var _local_3:* = (this.newPassword_.text().length >= 10);
            if (!_local_3)
            {
            ...
            {
                _local_1 = this.newPassword_.text();
                _local_2 = 0;
                while (_local_2 < (_local_1.length - 2))
                {
                    if (((_local_1.charAt(_local_2) == _local_1.charAt((_local_2 + 1))) && (_local_1.charAt((_local_2 + 1)) == _local_1.charAt((_local_2 + 2)))))
                    {
                        this.newPassword_.setError(TextKey.LINK_WEB_ACCOUNT_SHORT);
                        _local_3 = false;
                    }
