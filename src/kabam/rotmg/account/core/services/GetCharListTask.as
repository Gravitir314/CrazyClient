...
public class GetCharListTask extends BaseTask
    {
        ...
        {
            var _local_2:Number;
            var _local_3:MigrationDialog;
            var _local_4:XML;
            var _local_5:XML = new XML(_arg_1);
            if (_local_5.hasOwnProperty("MigrateStatus"))
            {
                _local_2 = _local_5.MigrateStatus;
                if (_local_2 == 5)
                {
                    this.sendRequest();
                }
                ...
                _local_3.cancel.addOnce(this.clearAccountAndReloadCharacters);
                this.openDialog.dispatch(_local_3);
            }
            else
            {
                if (_local_5.hasOwnProperty("Account"))
                {
                    if ((this.account is WebAccount))
                    {
                        WebAccount(this.account).userDisplayName = _local_5.Account[0].Name;
                        WebAccount(this.account).paymentProvider = _local_5.Account[0].PaymentProvider;
